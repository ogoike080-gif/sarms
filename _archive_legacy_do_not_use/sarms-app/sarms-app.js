...
function LoginPage({ onLogin, state, updateState }) {
  ...
  // Result checker state — 3 steps: class → student → term (no PIN needed)
  ...
  const handleChecker = () => {
    ...
    setCheckerResult({ classId: checkerClass, term: checkerTerm, student });
  };
        ...
        {mode === "login" ? (
          ...
          <>
            ...
            <button
              className="btn btn-primary"
              style={{ width: "100%", justifyContent: "center" }}
              disabled={!checkerClass || !checkerStudentId || !checkerTerm}
              onClick={handleChecker}
            >
              <Icon name="search" size={16} /> View My Result
            </button>
          </>
        )}
      </div>
...
function ResultCheckerView({ result, state, onBack }) {
  const { classId, term, student } = result;
