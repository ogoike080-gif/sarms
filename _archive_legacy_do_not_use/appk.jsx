// React hooks from global React object (loaded via CDN in index.html)
...
const DB = (() => {
  // Build the absolute URL to db.php regardless of subdirectory depth
  const BASE = (function() {
    const loc = window.location;
    // Get the path up to and including the /sarms/ folder
    const parts = loc.pathname.split('/');
    // Find 'sarms' folder name in path, or just use origin + /sarms
    const sarmsIdx = parts.findIndex(p => p === 'sarms');
    if (sarmsIdx >= 0) {
      const prefix = parts.slice(0, sarmsIdx + 1).join('/');
      return loc.origin + prefix + '/api/db.php';
    }
    // Fallback: assume we are in the root of the sarms folder
    return loc.origin + '/sarms/api/db.php';
  })();
...
function PageRouter({
  ...
  const pages = {
    ...
    students: <StudentsPage {...props} />,
    teachers: <TeachersPage {...props} />,
    classes: <ClassesPage {...props} />,
    broadsheet: isRestricted ? <DashboardPage {...props} /> : <BroadsheetPage {...props} />,
    myresult: isRestricted ? <DashboardPage {...props} /> : <MyResultPage {...props} />,
    pinmanager: <PINManagerPage {...props} />,
    audittrail: <AuditTrailPage {...props} />,
    institution: <InstitutionPage {...props} />,
    grading: <GradingPage {...props} />,
    scoreentry: <ScoreEntryPage {...props} />,
    assignments: <AssignmentsPage {...props} />,
...
function LoginPage({ onLogin, state, updateState }) {
  ...
  const handleChecker = () => {
    ...
    if (!checkerPin)       { setError("Please enter your PIN."); return; }
    ...
    if (!pinEntry) { setError("Invalid PIN. Please enter a valid PIN code."); return; }

    // PIN claimed by a different student?
    ...
    if (pinEntry.usedCount >= 3) {
      setError("This PIN has been used 3 times and is no longer valid."); return;
    }

    // All good — consume one use and claim if not yet claimed
    ...
    const usesLeft = 3 - (pinEntry.usedCount + 1); // remaining AFTER this use
        ...
        {mode === "login" ? (
          ...
          <>
            <div style={{ fontSize: 13, color: COLORS.textSecondary, marginBottom: 16, textAlign: "center" }}>
              Enter your details and PIN to view your result
            </div>
            ...
            {checkerTerm && (
              <div className="form-group">
                ...
                <input
                  ...
                  style={{ letterSpacing: "0.1em", fontWeight: 700, fontSize: 15 }}
                />
                <div style={{ fontSize: 11, color: COLORS.textMuted, marginTop: 5 }}>
                  🔑 Each PIN can be used 3 times only. Contact your school admin for a PIN.
                </div>
              </div>
            )}
...
function ResultCheckerView({ result, state, onBack }) {
  ...
  // Get scores for this specific student and selected term
  const scores = state.scores.filter(
    (s) => s.studentId === student.id && s.term === term && s.session === state.currentSession
  );

  // Rank among classmates so position shows correctly on result sheet
    ...
    <div className="login-page" style={{ flexDirection: "column", gap: 16, padding: "24px 16px" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap", maxWidth: 740, margin: "0 auto", width: "100%" }}>
        ...
        <div style={{
          ...
          <Icon name="pin" size={13} color={usesLeft === 0 ? COLORS.rose : usesLeft === 1 ? COLORS.gold : COLORS.emerald} />
          {usesLeft === 0
            ? "PIN exhausted — get a new PIN from admin"
    ...
    <div>
      <div className="section-header">
        <div>
          ...
          <div className="section-sub">{teachers.length} teachers</div>
        </div>
        <button className="btn btn-primary" onClick={() => setModal("add")}>
...
function ScoreEntryPage({ state, updateState, currentUser, showNotification }) {
  ...
  // Character report state: { [studentId]: { Punctuality: "Excellent", ... , _teacherRemark: "..." } }
  ...
  // Load character data when class/term changes
  useEffect(() => {
    const charReports = state.characterReports || {};
    const loaded = {};
    classStudents.forEach((st) => {
  ...
  // Selector bar used by both tabs
  const SelectorBar = () => (
...
root.render(React.createElement(SARMS));
